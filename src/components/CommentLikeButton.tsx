...
const CommentLikeButton = ({ postId, commentId, size = 'sm' }: CommentLikeButtonProps) => {
  const { isLiked, likeCount, loading, toggleLike } = useCommentLikes(postId, commentId)
  const navigate = useNavigate()
  
  console.log(`CommentLikeButton [comment-${commentId}] - isLiked: ${isLiked}, count: ${likeCount}`)

  const handleLikeClick = async (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    
    console.log(`댓글 좋아요 버튼 클릭 [comment-${commentId}]`)
